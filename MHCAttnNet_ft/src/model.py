...
class MHCAttnNet(nn.Module):
    ...
    def forward(self, peptide, mhc):
        ...
        '''
        peptide: [32, 45], 45 is the maximum of peptide input, e.g., [16,2,3,...1,1,1,1,1]
        self.peptide_embedding [23, 100] is basically a look-up table as a dictionary.
        If you wana check the values, use self.peptide_embedding.weight
        The output mhc_emb [32, 45, 100], the index of peptide first refer to amino acid, then refer to
        values in embedding. So that the followng LSTM traversing through the amnio acid sequence [1-45].
        '''
