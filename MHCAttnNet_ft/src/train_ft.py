...
from data_loader import get_dataset, IEDB_raw, IEDB_embed
...
if __name__ == "__main__":
    ...
    train_dataset = IEDB_embed(split="train", tokenizer_name='Rostlab/prot_bert') # max_length is only capped to speed-up example.
    val_dataset   = IEDB_embed(split="val",   tokenizer_name='Rostlab/prot_bert') # max_length is only capped to speed-up example.
    test_dataset  = IEDB_embed(split="test",  tokenizer_name='Rostlab/prot_bert') # max_length is only capped to speed-up example.
    ...
    # peptide_embedding = nn.Embedding(45, 1024)
    # peptide_embedding.weight = nn.Parameter(pep)
    # peptide_embedding.weight.required_grad = True
    #
    #
    # mhmhc_amino_acid_embeddingc_emb = nn.Embedding(350, 1024)
    # mhc_amino_acid_embedding.weight = nn.Parameter(mhc)
